...
export function renderPaymentSummary(){
  ...
  const deliveryOption= getDeliveryOption(cartItem.deliveryOptionId);
   ...
   .addEventListener('click',async()=>{//async so wait till fetch to finish
    try{
         const response =await fetch('https://supersimplebackend.dev/orders',{
        method: 'POST',
        headers:{//headers give more info about backend
       'Content-Type': 'application/json'
        },
        body :JSON.stringify({
          cart: cart// we cant send object directly hence stringify
        })
      });
    const order=  await response.json();//to wait for response to finish
    //console.log(order);
    addOrder(order);
    }catch(error){
